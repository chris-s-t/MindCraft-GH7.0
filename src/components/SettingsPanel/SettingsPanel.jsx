  ...
  return (
    <div className="futuristic-panel w-full rounded-lg p-4 pointer-events-auto border-t-2 flex flex-col h-full overflow-hidden" style={{ borderTopColor: 'var(--accent-color)' }}>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-2 mb-3">
        ...
        <h2 className="text-md font-mono font-bold tracking-widest text-[var(--accent-color)] uppercase">Sistem Pengaturan</h2>
      </div>

      <div className="space-y-4 text-sm text-slate-600 flex-1 overflow-y-auto pr-1">
        {/* 1. Accent Color Customizer */}
        <div>
          <label className="block text-xs uppercase tracking-wider text-slate-500 font-mono mb-2">Ambient Color Glow</label>
        ...
        <div>
          <label className="block text-xs uppercase tracking-wider text-slate-500 font-mono mb-2 flex items-center gap-1">
          ...
          <div className="space-y-2 bg-slate-50 border border-slate-200 rounded-lg p-2.5">
            ...
            <div className="pt-1.5 border-t border-slate-200 mt-1">
              <div className="flex justify-between text-[11px] mb-1 font-mono">
                <span className="text-slate-500">Jarak Peringatan:</span>
                <span className="text-[var(--accent-color)] font-bold">{alertRange} meter</span>
              </div>
              <input
                type="range"
                min="50"
                ...
                className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-[var(--accent-color)]"
              />
            </div>
          </div>
        </div>
                      ...
                      <div className="flex gap-1.5">
                        {[1, 3, 5].map((speed) => (
                          <button
                            ...
                            className={`px-2 py-0.5 rounded border text-[9px] font-bold transition cursor-pointer ${
                              simSpeed === speed
                                ? 'border-[var(--accent-color)] text-[var(--accent-color)] bg-[var(--accent-glow)]'
        ...
        {isSimulating && detectedHazard && (
          <div className="bg-red-50 border-2 border-red-400 rounded-lg p-3 flex items-start gap-3 animate-pulse shadow-sm">
            <ShieldAlert className="w-5 h-5 text-red-500 flex-shrink-0 mt-0.5" />
            <div className="font-mono text-xs min-w-0">
              <span className="text-red-600 font-bold block uppercase tracking-wider text-[10px] mb-0.5">⚠ PERINGATAN BAHAYA DI DEPAN!</span>
              <span className="text-slate-800 font-bold block truncate">{detectedHazard.title}</span>
              <span className="text-slate-500 block text-[10px] mt-0.5">Jarak: <span className="text-red-600 font-bold">{detectedHazard.distance}m</span></span>
            </div>
          </div>
        )}
      </div>
